...
def parse_args():
    ...
    parser.add_argument("--save_dir",  type=str, default="D:/EmbeddingsPoseAudio")
...
class StyleEmbeddingModel(nn.Module):
    def __init__(self, num_speakers, pose_dim=POSE_DIM, style_dim=FUSED_DIM, audio_dim=AUDIO_DIM, bottleneck_dim=BOTTLENECK_DIM, p=0):
        super().__init__()
        ...
        self.classifier = Classifier(in_dim=style_dim * 2, bottleneck_dim=bottleneck_dim, num_speakers=num_speakers)

    def forward(self, pose, audio_emb, label):
        pose_feat = self.pose_encoder(pose)
        audio_feat = self.audio_encoder(audio_emb)
        fused = torch.cat([pose_feat, audio_feat], dim=1)
...
def train_epoch(model, loader, optimiser):
    ...
    for vecs, labels, audio_embs in loader:
        vecs, labels, audio_embs = vecs.to(DEVICE), labels.to(DEVICE), audio_embs.to(DEVICE)
        optimiser.zero_grad()
        _, id_loss = model(vecs, audio_embs, labels)
        id_loss.backward()
        optimiser.step()
        with torch.no_grad():
            feat = torch.cat([model.pose_encoder(vecs), model.audio_encoder(audio_embs)], dim=1)
...
def main():
    ...
    # Model
    print("\nBuilding the Model")
    model = StyleEmbeddingModel(num_speakers=n_speakers, pose_dim=POSE_DIM, style_dim=FUSED_DIM, audio_dim=AUDIO_DIM, bottleneck_dim=BOTTLENECK_DIM).to(DEVICE)
    ...
    print("\nTraining")
    best_val_acc = 0.0
    best_path = os.path.join(SAVE_DIR, "pose_audio_style_best.pt")
    last_path = os.path.join(SAVE_DIR, "pose_audio_style_last.pt")
