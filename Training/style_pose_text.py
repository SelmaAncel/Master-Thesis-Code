...
def parse_args():
    ...
    parser.add_argument("--save_dir",  type=str, default="D:/EmbeddingsPoseText")
...
class GestureTextDataset(Dataset):
    def __init__(self, memmap_dirs, bert_pkl_paths, speaker_to_idx):
        ...
        for memmap_dir, bert_path in zip(memmap_dirs, bert_pkl_paths):
            ...
            print(f"Loaded memmap {memmap_dir}: {len(aux_info)} entries, shape {vec_seq.shape}, text {text_emb.shape}")
            for i, entry in enumerate(aux_info):
                vid = entry["vid"]
                ...
                self.samples.append((vec, spk_idx, text))
                counts[spk_idx] += 1
...
def train_epoch(model, loader, optimiser):
    ...
    for vecs, labels, bert_embs in loader:
        vecs, labels, bert_embs = vecs.to(DEVICE), labels.to(DEVICE), bert_embs.to(DEVICE)
        optimiser.zero_grad()
        _, id_loss = model(vecs, bert_embs, labels)
        id_loss.backward()
        optimiser.step()
        with torch.no_grad():
            feat = torch.cat([model.pose_encoder(vecs), model.text_encoder(bert_embs)], dim=1)
...
def main():
    ...
    # Model
    print("\nBuilding the Model")
    model = StyleEmbeddingModel(num_speakers=n_speakers, pose_dim=POSE_DIM, style_dim=FUSED_DIM, bert_dim=BERT_DIM, bottleneck_dim=BOTTLENECK_DIM).to(DEVICE)
    ...
    print("\nTraining")
    best_val_acc = 0.0
    best_path = os.path.join(SAVE_DIR, "pose_text_style_best.pt")
    last_path = os.path.join(SAVE_DIR, "pose_text_style_last.pt")
