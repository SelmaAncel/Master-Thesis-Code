...
SEED = 42
CHECKPOINT = "D:/EmbeddingsText/pose_text_90/mixstage_style_best.pt"
SPEAKER_MAP = "D:/EmbeddingsText/pose_text_90/speaker_to_idx.json"
...
MAX_UNSEEN_CLIPS = 20
SAVE_DIR = "D:/eval_results/pose_text/2s"
...
class ClassifierHead(nn.Module):
    def __init__(self, in_dim=512, bottleneck_dim=BOTTLENECK_DIM, num_speakers=1, hidden_dim=512):
        super().__init__()
...
class StyleEmbeddingModel(nn.Module):
    def __init__(self, num_speakers, pose_dim=POSE_DIM, style_dim=FUSED_DIM, bert_dim=BERT_DIM, bottleneck_dim=BOTTLENECK_DIM, p=0):
        super().__init__()
        ...
        self.classifier = ClassifierHead(in_dim=style_dim * 2, bottleneck_dim=bottleneck_dim, num_speakers=num_speakers)

    def forward(self, pose, text_emb, label):
        pose_feat = self.pose_encoder(pose)
        text_feat = self.text_encoder(text_emb)
        fused = torch.cat([pose_feat, text_feat], dim=1)
        logits, bottleneck = self.classifier(fused)
        id_loss = F.cross_entropy(logits, label)
        return bottleneck, id_loss

    @torch.no_grad()
    def get_embedding(self, pose, text_emb):
        pose_feat = self.pose_encoder(pose)
        text_feat = self.text_encoder(text_emb)
        fused = torch.cat([pose_feat, text_feat], dim=1)
...
class GestureTextDataset(Dataset):
    def __init__(self, memmap_dirs, bert_pkl_paths, speaker_to_idx):
        if isinstance(memmap_dirs, str):
            memmap_dirs = [memmap_dirs]
            bert_pkl_paths = [bert_pkl_paths]
        self.samples = []
        counts = defaultdict(int)
        for memmap_dir, bert_path in zip(memmap_dirs, bert_pkl_paths):
            ...
            for i, entry in enumerate(aux_info):
                ...
                self.samples.append((vec_seq[i].astype(np.float32).copy(), spk_idx, text_emb[i].astype(np.float32).copy()))
                counts[spk_idx] += 1
    ...
    def __getitem__(self, idx):
        vec, spk, bert_emb = self.samples[idx]
        return torch.from_numpy(vec), torch.tensor(spk, dtype=torch.long), torch.from_numpy(bert_emb)
...
def get_seen_embeddings(model, dataset):
    ...
    model.eval()
    for vecs, labels, bert_embs in loader:
        seen_embs.append(model.get_embedding(vecs.to(DEVICE), bert_embs.to(DEVICE)).cpu())
...
def get_unseen_embeddings(model, unseen_clips):
    ...
    for vid, clips in unseen_clips.items():
        ...
        bert_embs = torch.tensor(np.stack([c[1] for c in clips]), dtype=torch.float32)
        embs = model.get_embedding(vecs.to(DEVICE), bert_embs.to(DEVICE)).cpu().numpy()
        result[vid] = embs
    return result

# Load unseen clips
def load_unseen_clips(memmap_dirs, bert_pkl_paths, unseen_vids, max_clips_per_speaker):
    clips = defaultdict(list)
    counts = defaultdict(int)
    for d, bert_path in zip(memmap_dirs, bert_pkl_paths):
        aux = pickle.load(open(os.path.join(d, "aux_info.pkl"), "rb"))
        vecs = np.load(os.path.join(d, "vec_seq.npy"), mmap_mode="r")
        with open(bert_path, "rb") as f:
            bert_lookup = pickle.load(f)
        text_emb = np.zeros((len(aux), BERT_DIM), dtype=np.float32)
        ...
        for i, entry in enumerate(aux):
            ...
            if counts[vid] >= max_clips_per_speaker: 
                continue
            clips[vid].append((vecs[i].astype(np.float32).copy(), text_emb[i].astype(np.float32).copy()))
            counts[vid] += 1
...
def main():
    ...
    model = StyleEmbeddingModel(num_speakers = ckpt["n_speakers"], pose_dim = ckpt.get("pose_dim", POSE_DIM), style_dim = ckpt.get("fused_dim", FUSED_DIM), bert_dim = ckpt.get("bert_dim", BERT_DIM), bottleneck_dim = ckpt.get("bottleneck_dim", BOTTLENECK_DIM)).to(DEVICE)
    ...
    print("\nLoading dataset")
    full_dataset = GestureTextDataset([TRAIN_MEMMAP, VAL_MEMMAP, TEST_MEMMAP], bert_pkl_paths = [BERT_TRAIN_PKL, BERT_VAL_PKL, BERT_TEST_PKL], speaker_to_idx = speaker_to_idx)
    ...
    unseen_clips = load_unseen_clips([TRAIN_MEMMAP, VAL_MEMMAP, TEST_MEMMAP], [BERT_TRAIN_PKL, BERT_VAL_PKL, BERT_TEST_PKL], unseen_vids, MAX_UNSEEN_CLIPS)
