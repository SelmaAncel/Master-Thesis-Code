...
SEED = 42
CHECKPOINT = "D:/EmbeddingsAll/poseaudiotext_60/pose_audio_text_style_best.pt"
SPEAKER_MAP = "D:/EmbeddingsAll/poseaudiotext_60/speaker_to_idx.json"
...
MAX_UNSEEN_CLIPS = 20
SAVE_DIR = "D:/eval_results/pose_audio_text/2s"
        ...
        for memmap_dir, audio_path, bert_path in zip(memmap_dirs, audio_npy_paths, bert_pkl_paths):
            ...
            for i, entry in enumerate(aux_info):
                ...
                if np.abs(a_emb).sum() < 1e-6: 
                    continue 
                self.samples.append((vec_seq[i].astype(np.float32).copy(), spk_idx, text_emb[i].astype(np.float32).copy(), a_emb.astype(np.float32).copy()))
                counts[spk_idx] += 1
...
def get_seen_embeddings(model, dataset):
    ...
    for vecs, labels, bert_embs, audio_embs in loader:
        seen_embs.append(model.get_embedding(
            vecs.to(DEVICE), bert_embs.to(DEVICE), audio_embs.to(DEVICE)).cpu())
...
def get_unseen_embeddings(model, unseen_clips):
    ...
    for vid, clips in unseen_clips.items():
        ...
        audio_embs = torch.tensor(np.stack([c[2] for c in clips]), dtype=torch.float32)
        embs = model.get_embedding(vecs.to(DEVICE), bert_embs.to(DEVICE), audio_embs.to(DEVICE)).cpu().numpy()
        result[vid] = embs
    return result

# Load unseen clips
def load_unseen_clips(memmap_dirs, audio_npy_paths, bert_pkl_paths, unseen_vids, max_clips_per_speaker):
    clips = defaultdict(list)
    counts = defaultdict(int)
    for d, audio_path, bert_path in zip(memmap_dirs, audio_npy_paths, bert_pkl_paths):
        ...
        for i, entry in enumerate(aux):
            ...
            if np.abs(a_emb).sum() < 1e-6:          
                continue 
            clips[vid].append((vecs[i].astype(np.float32).copy(), text_emb[i].astype(np.float32).copy(), a_emb.astype(np.float32).copy()))
            counts[vid] += 1
...
def main():
    ...
    model = StyleEmbeddingModel(num_speakers = ckpt["n_speakers"], pose_dim = ckpt.get("pose_dim", POSE_DIM), style_dim = ckpt.get("fused_dim", FUSED_DIM),
        bert_dim = ckpt.get("bert_dim", BERT_DIM), audio_dim = ckpt.get("audio_dim", AUDIO_DIM), bottleneck_dim = ckpt.get("bottleneck_dim", BOTTLENECK_DIM)).to(DEVICE)
    ...
    unseen_clips = load_unseen_clips([TRAIN_MEMMAP, VAL_MEMMAP, TEST_MEMMAP], [AUDIO_TRAIN_NPY, AUDIO_VAL_NPY, AUDIO_TEST_NPY], [BERT_TRAIN_PKL, BERT_VAL_PKL, BERT_TEST_PKL], unseen_vids, MAX_UNSEEN_CLIPS)
